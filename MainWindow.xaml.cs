...
namespace TimeUpdater
{
    ...
    public partial class MainWindow : Window
    {
        ...
        {
            var lastSaveDateTime = _filesManager.GetLastSavedTime();
            if(lastSaveDateTime.HasValue)
            {
                MessageBox.Show(lastSaveDateTime.Value.ToString(CultureInfo.CurrentCulture), "Last saved time", MessageBoxButton.OK, MessageBoxImage.Information);
            }
        }

        private void SetStatusLabel()
        {
            DateTime? lastSavedDateTemp = _filesManager.GetLastSavedTime();

            if(!lastSavedDateTemp.HasValue)
            {
                statusLabel.Content = "Status: No saved date found!";
                statusLabel.Foreground = Brushes.Red;
                return;
            }

            var lastSavedDate = lastSavedDateTemp.Value;
            DateTime today = DateTime.Today;
            string statusText;
            Brush statusColor;

            bool isYesterday = lastSavedDate.Date == today.AddDays(-1);
            bool isLastSaveOnFriday = today.DayOfWeek == DayOfWeek.Monday && lastSavedDate.Date == today.AddDays(-3) && lastSavedDate.DayOfWeek == DayOfWeek.Friday;

            if(isYesterday || isLastSaveOnFriday)
            {
                statusText = $"Status: OK  (last saved date: {lastSavedDate:yyyy-MM-dd})";
                statusColor = Brushes.Green;
            }
            else
            {
                statusText = $"Status: Last workday is potentially not stored! (last saved date: {lastSavedDate:yyyy-MM-dd})";
