...
namespace TimeUpdater
{
    public class FilesManager
    {
        ...
        /// <summary>
        /// Get last saved time of work day as DateTime 
        /// </summary>
        /// <returns></returns>
        public DateTime? GetLastSavedTime()
        {
            var unixTime = GetLastSavedTimeUnixFormat();
            if(unixTime > 0)
            {
                return TimeConverter.ConvertUnixTimeToDateTime(unixTime);
            }

            return null;
        }

        private int GetLastSavedTimeUnixFormat()
        {
